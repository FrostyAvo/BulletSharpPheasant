                ...
                using (var colObjStream = new MemoryStream(colObjData, false))
                {
                    using (var colObjReader = new BulletReader(colObjStream))
                    {
                        if ((file.Flags & FileFlags.DoublePrecision) != 0)
                        {
                            ...
                            Matrix4 startTransform = colObjReader.ReadMatrixDouble(CollisionObjectDoubleData.Offset("WorldTransform"));
                        ...
                        else
                        {
                            ...
                            CollisionShape shape = _shapeMap[shapePtr];
                            Matrix4 startTransform = colObjReader.ReadMatrix(CollisionObjectFloatData.Offset("WorldTransform"));
